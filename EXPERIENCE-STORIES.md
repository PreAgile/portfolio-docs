...
### "PG사/외부 API 연동 경험이 있나요?" — 솔직하게

> "PG사 직접 연동 경험은 없습니다.
> 다만, 외부 API가 불안정한 환경에서 서킷 브레이커, Rate Limiting, 장애 격리를
> 직접 설계하고 운영한 경험이 있고, **문제 패턴은 동일**합니다.
> 구체적으로:
> - 외부 서비스 장애 시 연쇄 실패 방지 → 서킷 브레이커
> - 요청량 제어 → 토큰 버킷 Rate Limiting
> - 장애 서비스 우회 → 가중치 기반 라우팅 (Fallback 전환)
> - 배포 시 인플라이트 요청 보호 → Graceful Shutdown
>
> PG사 연동은 경험하지 않았지만, 같은 패턴의 문제를 더 열악한 환경
> (API 스펙 문서 없음, 언제든 차단 가능)에서 풀어본 경험이 있습니다."

### 이 경험이 귀사에서 어떻게 전이되는가 (클로징)

| 분산 시스템 패턴 | 내 경험 | 귀사에서 쓰이는 곳 |
|---------------|--------|------------------|
| 서킷 브레이커 + 점진적 복구 | 외부 플랫폼 장애 대응 | 외부 API/PG사 장애 격리 |
| 가중치 기반 라우팅 | 프록시 건강도 스코어링 | 서비스 메시 LB, 멀티 DB 라우팅 |
| 리소스 풀 관리 | 브라우저 30+ 인스턴스 | DB 커넥션 풀, 스레드 풀, Pod 관리 |
| Lease 기반 분산 큐 | 세션별 요청 직렬화 | 분산 락, 작업 큐, 이벤트 순서 보장 |
| Graceful Shutdown | 드레인 모드 + 좀비 정리 | K8s 무중단 배포, 롤링 업데이트 |
| 에러 분류 체계 | 4단계 Severity | Observability, 알림 파이프라인 |
| TLS/프로토콜 디버깅 | Akamai 핑거프린트 분석 | HTTPS/mTLS 이슈 트러블슈팅 |
