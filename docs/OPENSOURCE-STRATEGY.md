...
| **Resilience4j** | #2383 (Aspect Order) or #2189 (ConcurrentModification) | Java + Spring AOP + 서킷 브레이커 = 최고 조합 |
| **Spring Kafka** | #4384 (ConsumerFactory 버그) | Kafka + Spring 통합 이해 증명 |
...
|---------|---------|------|
| **라인** | Armeria | KafkaLogWriter or throttling strategy |
| **토스/당근** | kotest | #3705 Coroutine scheduler 상속 (Hard) |
| **우아한** | Exposed or Spring Data | 트랜잭션/배치 관련 이슈 |

### Phase 4 (6주~): 소스 분석만 하는 프로젝트

| 프로젝트 | 분석 목적 | 면접 활용 |
|---------|---------|---------|
| **Spring Framework** TransactionInterceptor | @Transactional 프록시 동작 | "소스에서 CGLIB 프록시 생성 과정을 확인했습니다" |
| **HikariCP** ConcurrentBag | 커넥션 풀 내부 동작 | "ConcurrentBag의 threadLocal + sharedList 전략을 분석했습니다" |
| **Redisson** RedissonLock | 분산 락 Lua 스크립트 | "SET NX로 직접 구현한 것과 Redisson watchdog 비교 분석" |
| **kotlinx.coroutines** | CPS 변환, Dispatcher 구현 | "Coroutine이 어떻게 suspend → resume되는지 바이트코드 레벨로" |
...
| 항목 | 현재 | 목표 | 비고 |
|------|:---:|:---:|------|
| 외부 오픈소스 총 PR | 10개 | **15~18개** | +5~8개 |
| 코드 레벨 PR (docs 제외) | 8개 | **13~15개** | +5~7개 |
| Hard 난이도 PR | 3개 | **5~6개** | +2~3개 |
| 기여한 프로젝트 수 | 5개 | **8~9개** | +3~4개 (Resilience4j, Spring Kafka, Armeria 등) |
| Java/Spring 프로젝트 PR | 2개 (docs만) | **4~5개 (코드 포함)** | 핵심 갭 해소 |
| Kotlin 프로젝트 PR | 7개 | **10~12개** | kotest 확장 + Exposed |

**이 수치를 달성하면:**
- "Kotlin 생태계에 10개+ PR" → 토스/당근/우아한 면접에서 Kotlin 역량 의심 불가
- "Resilience4j + Spring Kafka 코드 PR" → Java/Spring 코드 레벨 이해 증명
- "Armeria PR" → 라인 면접 강력 시그널
- "총 15~18개 머지 PR" → 시니어 레벨 오픈소스 기여자로 포지셔닝
