...
| cmong-scraper-js | NestJS + Playwright/Camoufox + Dual MySQL + Redis | 서킷 브레이커(적응형 트래픽 제어), 프록시 건강도 스코어링, 브라우저 풀 관리, Akamai TLS 핑거프린트 우회, 좀비 프로세스 탐지, Graceful Shutdown |
...
#### 4-2-2. async-crawler 구현
- [ ] Kotlin Coroutine + Spring Batch
  - cmong-scraper의 동시성 패턴을 Coroutine으로 재설계
  - `withContext(Dispatchers.IO)` 블로킹 I/O 격리
  - `SupervisorJob` 장애 격리
- [ ] Resilience4j 서킷 브레이커
  - cmong-scraper의 Adaptive Traffic Controller를 Resilience4j로 재구현
  - 설정값(에러 임계값, 슬로우콜 임계값) 근거 문서화
- [ ] Rate Limiting
...
- cmong-scraper 서킷 브레이커 설정값 근거 문서화 → 면접 스토리 소재
...
| 대상 | 왜 이것인가 | 실무 연결점 | 목표 |
|------|-----------|-----------|------|
| **Resilience4j** | 서킷 브레이커 — cmong-scraper 적응형 트래픽 제어와 직결 | "직접 구현한 서킷 브레이커와 비교 분석" | 소스 분석 + 이슈/PR 1개 |
...
| 서킷 브레이커 설정 근거 | cmong-scraper | "에러율 임계값과 슬로우콜 기준을 실측 데이터 기반으로 최적화" | 낮음 |
