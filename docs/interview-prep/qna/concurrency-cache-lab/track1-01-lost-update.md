...
```
어드민에서 "답글 일괄 등록" → reply_requests 에 PENDING 상태로 N건 저장
   → RabbitMQ에 N개 메시지 발행
   → 30+ 컨슈머 인스턴스가 메시지 분산 처리
   → 각 컨슈머가 processReply(id) 호출:
      1. reply 조회
      2. PENDING → PROCESSING + retry_count += 1
      3. 외부 플랫폼 API 호출 (진짜 댓글 등록)
      4. 성공 시 COMPLETED
```

### 동시성 경합이 실제로 발생하는 이유

1. **RabbitMQ at-least-once**: 네트워크 ack 실패 시 같은 메시지가 다른 컨슈머에 재배달
2. **멀티 인스턴스**: 30+ 컨슈머 인스턴스가 같은 큐를 바라봄
3. **수동 재시도**: 어드민에서 "재처리" 버튼 중복 클릭
...
3. **분산 환경 경합이 자연스럽게 발생**: RabbitMQ at-least-once + 멀티 인스턴스
...
댓글 처리가 **외부 API 호출 + 상태 머신 + 멀티 인스턴스 컨슈머**를 모두 포함해서 현실성이 가장 높음.
...
> "운영 중인 B2B SaaS의 댓글 처리 컨슈머에 Redis SETNX 기반 messageId 락이 이미 구현되어 있습니다. `acquireProcessingLock(messageId)`으로 첫 컨슈머만 처리하고, 나머지는 nack로 재큐합니다. 이 락이 없었을 때 외부 플랫폼에 답글 중복 등록 사고가 있었고, 그 사고를 단순화해서 재현 가능한 형태로 만든 게 이 실험입니다."
...
> "운영 중인 댓글 등록 컨슈머는 **Redis SETNX + messageId 단위 락**으로 1차 방어를 하고, 외부 플랫폼 API 응답에서 실패 코드가 반환되면 **이미 등록된 답글인지 조회 API로 재확인** 후 재시도합니다. 순수 락만으로는 부족해서 '락 + 외부 상태 검증' 이중 방어를 구축했습니다.
>
...
>
> 이후 **Redis SETNX messageId 락**으로 전환. 낙관적 락은 '충돌 감지'에 맞고, 고부하 상황에서는 '충돌 자체를 막는' 전략이 맞았습니다.
>
...
> "실제 운영 시스템은 이 구조입니다:
> 1. RabbitMQ 메시지의 `messageId`를 키로 Redis SETNX (30초 TTL)
> 2. 락 획득한 컨슈머만 처리, 나머지는 nack + 재큐
> 3. 외부 API는 **답글 등록 전 조회 API로 현재 상태 재확인** (멱등성 보강)
> 4. 실패 시 DLQ(Dead Letter Queue)로 이동해 수동 검토
>
...
이 실험은 본인이 운영 중인 시스템의 `acquireProcessingLock(messageId)`가 **왜 존재해야 하는지**를 재현 가능한 형태로 정리한 것. 면접에서:
