...
**경로 A — on-demand (사용자 트리거, batchjob.service.ts:28)**
```
[사용자 액션 → cmong-be]
  batchjobService.populateShop()
       │ HTTP POST http://mq.lemong.ai:8000/populate
       ▼
[cmong-mq / FastApi.py]
...
**경로 B — 배치 큐 (cmong-be 내부 RabbitMQ, queue.consumer.ts)**
```
...
**producer.py 내부 병렬 처리 구조 (producer.py:568)**
```python
...
EC2 1대(LemongConsole)에서 플랫폼별로 crontab을 통해 독립 프로세스로 실행한다:
...
- cmong-mq/mqscript.sh: 플랫폼별 worker 수, cron 실행 구조
- cmong-mq/mqscript_hyphen.sh: CPEATS Hyphen API 별도 실행 경로
- cmong-mq/scripts/cpeats_crontab_example.sh: CPEATS 세션 유지 크론
- cmong-mq/producer.py: ThreadPoolExecutor + queue.Queue 실제 처리 구조
- cmong-mq/FastApi.py: subprocess.run 블로킹 호출 구조
- cmong-mq/constants.py: PLATFORM_LOGIN_ERROR_COUNTS, ERROR_RATIO_THRESHOLDS
