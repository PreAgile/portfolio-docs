...
이 질문은 **반드시 나옵니다.** 숨기거나 돌리지 않습니다.

**원칙: 스크래핑 자체의 난이도도 인정하고, 분산 시스템 설계도 어필한다. 둘 다 내 경험이다.**

> "네, 웹 스크래핑 기반 데이터 수집 파이프라인입니다.
> 이 시스템에서 기술적으로 어려운 부분은 **두 가지 축**이 있었습니다."

**축 1: 스크래핑 자체의 기술적 난이도 (솔직하게)**

> "외부 플랫폼이 Akamai Bot Manager를 도입하면서 TLS 핑거프린트 레벨에서 클라이언트를 식별했습니다.
> 단순한 헤더 변경이나 User-Agent 위조로는 해결이 안 됐고,
> 브라우저 엔진을 Chromium에서 Firefox 기반(Camoufox)으로 교체하고,
> Canvas/WebGL 노이즈 주입, navigator 속성 위조 등 핑거프린트 스푸핑을 구현했습니다.
> 이 과정에서 **TLS 핸드셰이크, 브라우저 렌더링 엔진, HTTP 프로토콜 내부 동작**을
> 깊이 이해하게 됐습니다."

**축 2: 그것을 안정적으로 운영하기 위한 분산 시스템 설계**

> "하지만 더 큰 챌린지는 이 스크래핑을 **안정적으로 대규모 운영**하는 것이었습니다:
> - 30+ 인스턴스의 **리소스 풀 라이프사이클 관리**
> - 에러율 기반 **3단계 서킷 브레이커** (Redis Lua로 다중 인스턴스 상태 공유)
> - 성공률/차단율/지연 기반 **가중치 라우팅**
> - **Lease 기반 분산 큐**로 세션별 요청 직렬화
> - **Graceful Shutdown** (드레인 모드 + 좀비 프로세스 정리)
>
> 이 패턴들은 스크래핑에 특화된 게 아니라,
> 외부 의존성이 불안정한 모든 분산 시스템에 적용되는 범용 패턴입니다."

### "외부 API 연동 경험이 있나요?" — 자신 있게

> "네, 6개 외부 플랫폼 API를 직접 연동하고 운영한 경험이 있습니다.
> 플랫폼마다 인증 방식이 다르고(세션 기반, OAuth, 쿠키 기반),
> API 스펙이 사전 통보 없이 변경되거나, Rate Limit이 갑자기 강화되는 환경에서
> 서킷 브레이커, 에러 분류, 재시도 전략, 장애 격리를 직접 설계했습니다.
>
> 구체적으로:
> - 플랫폼별 인증/세션 관리 (토큰 갱신, 만료 감지, 자동 재로그인)
> - API 스펙 변경 시 런타임 대응 (응답 포맷 변경 감지 → 알림 → 핫픽스)
> - 에러율 기반 서킷 브레이커 (50%/30%/15% 3단계)
> - 가중치 기반 라우팅 (성공률/지연/차단율 스코어링)
> - Graceful Shutdown (배포 시 인플라이트 요청 보호)"

### "PG사 직접 연동 경험은?" — 솔직하게

> "PG사를 직접 연동한 경험은 없습니다.
> 결제 시스템에서는 Portone(PG 게이트웨이)을 통해 결제/구독/웹훅을 처리했고,
> Portone 뒤의 PG사(이니시스, NHN 등)를 직접 연동하진 않았습니다.
>
> 다만 외부 API 연동에서 겪는 핵심 문제 — 장애 격리, Rate Limit 준수, 에러 분류,
> 재시도 전략, 멱등성 보장 — 이런 것들은 6개 플랫폼 API 운영에서 직접 풀어본 경험이 있고,
> PG사 연동과 **문제 패턴이 동일**합니다.
> 오히려 API 스펙 문서가 없고 언제든 차단될 수 있는 환경이라
> PG사 연동보다 더 열악한 조건에서 안정성을 보장해야 했습니다."

### 이 경험이 귀사에서 어떻게 전이되는가 (클로징)

| 분산 시스템 패턴 | 내 경험 | 귀사에서 쓰이는 곳 |
|---------------|--------|------------------|
| **외부 API 연동 + 장애 대응** | 6개 플랫폼 API (인증, 세션, 토큰, Rate Limit) | PG사 연동, 외부 마이크로서비스 호출 |
| 서킷 브레이커 + 점진적 복구 | 에러율 기반 3단계 자동 전환 | 외부 API 장애 격리, Fallback |
| 가중치 기반 라우팅 | 성공률/차단율/지연 스코어링 | 서비스 메시 LB, 멀티 DB 라우팅 |
| 리소스 풀 관리 | 30+ 인스턴스 라이프사이클 | DB 커넥션 풀, 스레드 풀, Pod 관리 |
| Lease 기반 분산 큐 | 세션별 FIFO + TTL 자동 해제 | 분산 락, 작업 큐, 이벤트 순서 보장 |
| Graceful Shutdown | 드레인 모드 + 좀비 정리 | K8s 무중단 배포, 롤링 업데이트 |
| 에러 분류 체계 | 4단계 Severity + 임계값 알림 | Observability, 알림 파이프라인 |
| TLS/프로토콜 디버깅 | WAF 핑거프린트 분석 + 우회 | HTTPS/mTLS 이슈 트러블슈팅 |
...
## Episode 4: MQ 기반 비동기 데이터 수집 + 에러 복구 체계

### 상황
6개 외부 플랫폼에서 리뷰/주문 데이터를 수집하는 파이프라인.
외부 API 호출은 실패가 잦고(네트워크, 세션 만료, 차단, API 스펙 변경 등), 처리 시간도 불균일(1초~5분).
...
> 플랫폼마다 안정성이 다릅니다. 특정 플랫폼은 WAF(Web Application Firewall) 때문에
> 정상 상황에서도 5~10% 실패가 발생하므로 임계값을 15%로 높였고,
> 안정적인 플랫폼은 10%로 설정했습니다.
